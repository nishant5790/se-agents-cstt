...
from .ckm import CKM, ContentBlock, Edge, KnowledgeGraph, Node
...
# Blocks are labelled by the LLM in batches to keep call volume low on large
# corpora. Tune via env; 0 disables the LLM and uses keyword labelling only.
_BATCH = int(os.getenv("UNDERSTANDING_BATCH", "25"))
_MAX_LLM_BLOCKS = int(os.getenv("UNDERSTANDING_MAX_LLM_BLOCKS", "300"))
...
class UnderstandingAgent(Agent):
    ...
    def _label_blocks(self, blocks: list[ContentBlock]) -> dict[str, tuple[str, list[str]]]:
        ...
        use_llm = llm.available() and _BATCH > 0 and _MAX_LLM_BLOCKS > 0
        llm_budget = _MAX_LLM_BLOCKS if use_llm else 0
        total = len(blocks)
        self.log(
            f"labelling {total} block(s) — mode={'LLM+keyword' if use_llm else 'keyword-only'}, "
            f"batch={_BATCH}, llm_budget={llm_budget}"
        )

        i = 0
        batch_no = 0
        while i < len(blocks) and llm_budget > 0:
            batch = blocks[i:i + min(_BATCH, llm_budget)]
            batch_no += 1
            self.log(
